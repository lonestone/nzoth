...
describe('typed-param', () => {
  ...
  describe('test uuid', () => {
    ...
    it('should reject invalid uuid', async () => {
      await request(app.getHttpServer()).get('/typed-param/not-a-uuid').expect(400);
    });
  });
  ...
  describe('test boolean', () => {
    ...
    it('should reject numeric boolean', async () => {
      await request(app.getHttpServer()).get('/typed-param/boolean/1').expect(400);
    });

    it('should reject empty value', async () => {
      await request(app.getHttpServer()).get('/typed-param/boolean/').expect(400);
    });
  });
  ...
  describe('test string', () => {
    ...
    it('should reject empty string', async () => {
      await request(app.getHttpServer()).get('/typed-param/string/').expect(400);
    });
  ...
  describe('test number', () => {
    it('should accept integer', async () => {
      const response = await request(app.getHttpServer()).get('/typed-param/number/42').expect(200);
    ...
    it('should accept zero', async () => {
      const response = await request(app.getHttpServer()).get('/typed-param/number/0').expect(200);
  ...
  describe('test int', () => {
    it('should accept integer', async () => {
      const response = await request(app.getHttpServer()).get('/typed-param/int/42').expect(200);

      expect(response.body).toEqual({ value: 42 });
    });

    it('should reject decimal', async () => {
      await request(app.getHttpServer()).get('/typed-param/int/3.14').expect(400);
    });
  ...
  describe('test multiple', () => {
    ...
    it('should reject when first param is invalid', async () => {
      await request(app.getHttpServer()).get('/typed-param/multiple/not-a-uuid/42').expect(400);
    });
