...
const defaultSchema = createFilterQueryStringSchema(['email', 'age', 'active'])

@Controller('filtering')
class TestController {
  @Get()
  getDefault(@FilteringParams(defaultSchema) filtering: Filtering[] | undefined) {
    return filtering
  }
}
...
describe('filtering', () => {
  ...
  describe('createFilterQuerySchema function', () => {
    ...
    it('should reject filters with invalid properties', () => {
      ...
      const result = schema.safeParse('invalid:eq:value')
      expect(result.error?.issues).toEqual([
        {
          code: 'invalid_property',
          message: 'Invalid filtering property. Allowed properties are: name',
          path: [0, 'property'],
        },
      ])
    })
  ...
  describe('filter rules', () => {
    ...
    it('should handle LIKE rule', async () => {
      ...
      expect(response.body).toEqual([
        { property: 'email', rule: FilterRule.LIKE, value: 'test' },
      ])
    })
    ...
    it('should handle IN rule with multiple values', async () => {
      ...
      expect(response.body).toEqual([
        { property: 'age', rule: FilterRule.IN, value: '20,25,30' },
      ])
    })
  })
