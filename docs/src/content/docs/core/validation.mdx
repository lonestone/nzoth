...
@TypedRoute.Get()
search(
  @TypedQuery('q', z.string().min(2), { optional: true }) q?: string,
) { /* ... */ }
...
`TypedQuery` handles one parameter (with `{ array, optional }`), while `TypedQueryObject` validates the full query object and expands Swagger entries.
