...
- `TypedMultipartBody(schema)` — validate `multipart/form-data` bodies
...
Ensures `Content-Type: multipart/form-data`, validates with Zod, and registers an `ApiBody` schema.
